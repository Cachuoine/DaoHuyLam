...
----------------------------------------------------------------------
local CorrectKey = "FishHub_2026_FreeKey" -- Bạn có thể đổi key ở đây
local KeyFileName = "FishHub_Key.json"
local SavedKey = ""

if readfile and isfile and isfile(KeyFileName) then
    pcall(function()
        local data = HttpService:JSONDecode(readfile(KeyFileName))
...
-- Tạo GUI tổng chứa cả Key System và Main UI
...
-- Hàm khởi chạy toàn bộ giao diện chính của bạn (được gọi khi vượt qua màn Key)
local StartMainHub;
...
KeyScreen.Size = UDim2.new(0, 400, 0, 240)
KeyScreen.Position = UDim2.new(0.5, 0, 0.5, 0)
KeyScreen.AnchorPoint = Vector2.new(0.5, 0.5)
KeyScreen.BackgroundColor3 = Color3.fromRGB(16, 16, 20)
KeyScreen.BorderSizePixel = 0

Instance.new("UICorner", KeyScreen).CornerRadius = UDim.new(0, 14)
local keyStroke = Instance.new("UIStroke")
keyStroke.Parent = KeyScreen
keyStroke.Thickness = 2
keyStroke.Color = Color3.fromRGB(0, 229, 255)
...
keyTitle.Text = "🔑 FishHub - Key System"
...
submitBtn.Parent = KeyScreen
submitBtn.Size = UDim2.new(1, -40, 0, 38)
submitBtn.Position = UDim2.new(0, 20, 0, 120)
submitBtn.BackgroundColor3 = Color3.fromRGB(0, 229, 255)
submitBtn.Font = Enum.Font.GothamBold
submitBtn.Text = "CHECK KEY"
submitBtn.TextSize = 13
submitBtn.TextColor3 = Color3.fromRGB(16, 16, 20)
Instance.new("UICorner", submitBtn).CornerRadius = UDim.new(0, 8)

local getKeyBtn = Instance.new("TextButton")
getKeyBtn.Parent = KeyScreen
getKeyBtn.Size = UDim2.new(0.47, -10, 0, 32)
...
local discordKeyBtn = Instance.new("TextButton")
discordKeyBtn.Parent = KeyScreen
discordKeyBtn.Size = UDim2.new(0.47, -10, 0, 32)
discordKeyBtn.Position = UDim2.new(0.51, 0, 0, 175)
discordKeyBtn.BackgroundColor3 = Color3.fromRGB(35, 35, 45)
discordKeyBtn.Font = Enum.Font.GothamBold
discordKeyBtn.Text = "Discord"
discordKeyBtn.TextSize = 12
discordKeyBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
Instance.new("UICorner", discordKeyBtn).CornerRadius = UDim.new(0, 6)

getKeyBtn.MouseButton1Click:Connect(function()
    pcall(function()
        if setclipboard then
            setclipboard("https://linkvertise.com/example-get-key") -- Đổi link lấy key của bạn ở đây
...
discordKeyBtn.MouseButton1Click:Connect(function()
    pcall(function()
        if setclipboard then
            setclipboard("https://discord.gg/2tTJxRk2ct")
            discordKeyBtn.Text = "Copied Discord!"
            task.wait(1.5)
            discordKeyBtn.Text = "Discord"
        end
    end)
...
-- Xử lý logic nút Kiểm tra Key
submitBtn.MouseButton1Click:Connect(function()
    if keyBox.Text == CorrectKey then
        submitBtn.Text = "SUCCESS!"
        submitBtn.BackgroundColor3 = Color3.fromRGB(50, 230, 80)
        
        -- Lưu key lại
        if writefile then
            pcall(function()
                writefile(KeyFileName, HttpService:JSONEncode({key = keyBox.Text}))
            end)
        end
        
        task.wait(0.5)
        KeyScreen:Destroy()
        StartMainHub() -- Kích hoạt giao diện chính của bạn
    else
        submitBtn.Text = "INVALID KEY!"
        submitBtn.BackgroundColor3 = Color3.fromRGB(230, 50, 50)
        task.wait(1)
        submitBtn.Text = "CHECK KEY"
        submitBtn.BackgroundColor3 = Color3.fromRGB(0, 229, 255)
    end
...
-- Nếu trong file đã có sẵn Key đúng từ trước, tự động vượt qua bảng Key
...
----------------------------------------------------------------------
-- TOÀN BỘ GIAO DIỆN CHÍNH CỦA BẠN (ĐƯỢC GÓI TRONG HÀM StartMainHub)
----------------------------------------------------------------------
StartMainHub = function()
